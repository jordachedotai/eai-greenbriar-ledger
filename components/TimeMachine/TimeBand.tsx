...
// the room. "Add April reports" streams the reading log and moves the
// month forward. Absent at the last month of the ledger.
  ...
  return (
    <div className="flex h-9 shrink-0 items-center gap-4 border-b px-7 text-[14px]" style={{ background: "#fbf3dd", borderColor: "#f0e2b8", color: "#8a5a08" }} data-testid="time-band" data-month={state.month}>
      ...
      <span className="ml-auto flex items-center gap-4">
        <button type="button" onClick={() => addNextReports()} disabled={busy} className="rounded-[7px] bg-[#8a5a08] px-2.5 py-[3px] text-[13px] font-semibold text-white hover:bg-[#6f4806] disabled:opacity-60" data-testid="time-band-add">
          {reading ? `Reading the ${monthLabel(reading.to)} reports` : `Add ${monthLabel(unread[0])} reports`}
        </button>
        <button type="button" onClick={() => setOpen(true)} disabled={busy} className="inline-flex items-center gap-1.5 font-semibold hover:underline disabled:opacity-60" data-testid="time-band-open">
          <IconClockBack size={15} />
          Go back in time
        </button>
        <button type="button" onClick={() => setCutoff(LAST_MONTH)} disabled={busy} className="font-semibold underline decoration-dotted underline-offset-[3px] hover:decoration-solid disabled:opacity-60" data-testid="time-band-return">
