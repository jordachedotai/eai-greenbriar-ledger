...
- `data/demo-states.json`: one state per company set (`core`, `all`) per cutoff month, keyed `core-2026-07` and so on. Generated by `scripts/gen-states.ts` from the fixtures above. The named states are presets in `lib/states.ts`: `july` is `core-2026-07`, `august` is `core-2026-08`, `august-approved` is `august` with Harlan's questions approved, `monday` is `all-2026-08`. The header's month scrubber moves the cutoff. Each core initiative carries an authored `statusAt` sentence per month in `arcs.json`; the nine extra companies read the month's verbatim sentence instead. `monday` shows all twelve companies: the core three plus nine more fictional ones with two initiatives each and one-line monthly sentences, every cell a quote with a cite, most green throughout, one amber month in three of them, one grey month in one. Their reports are one page each. No questions, patterns, or notes for the nine.
